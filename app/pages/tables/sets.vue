<template>
    <div class="table-container">
        <smart-table v-if="loading === 0"></smart-table>
        <loading v-else></loading>
        <mdl-dialog ref="editModal" :title="editMode && editRow._id?'Редактирование записи':'Добавление записи'" :noFocusTrap="true">
...
<script>
  ...
  export default {
    computed: {
      ...
      ...mapState('table', ['query', 'rows', 'info', 'loading', 'model', 'options', 'editRow', 'structure']),
      ...mapGetters('table', ['count', 'sortBy', 'sortDirection', 'editMode']),
    },
    methods: {
      ...mapMutations('table', ['SET_EDIT_MODAL']),
    ...
    mounted() {
      this.SET_EDIT_MODAL(this.$refs.editModal);
    }
  };
</script>
